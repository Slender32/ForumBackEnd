...
@Tag(name = "AdminComment", description = "管理端评论管理")
...
class AdminCommentController(
    ...
    @GetMapping
    @Operation(summary = "查询评论", description = "需要登录并具有 comment:manage 权限。")
    ...
    @GetMapping("/{id}")
    @Operation(summary = "查询详情：评论", description = "需要登录并具有 comment:manage 权限。")
    @ApiResponse(responseCode = "200", description = "操作成功，code 为 0；data 结构见响应模型", useReturnTypeSchema = true)
    fun get(
    ...
    @PutMapping("/{id}")
    @Operation(summary = "替换评论", description = "需要登录并具有 comment:manage 权限。")
    @ApiResponse(responseCode = "200", description = "操作成功，code 为 0；data 结构见响应模型", useReturnTypeSchema = true)
    fun replace(
        @Parameter(description = "评论 ID")
        @PathVariable id: Long,
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "删除评论", description = "需要登录并具有 comment:manage 权限。")
    @ApiResponse(responseCode = "200", description = "操作成功，code 为 0；data 结构见响应模型", useReturnTypeSchema = true)
    fun delete(
        @Parameter(description = "评论 ID")
        @PathVariable id: Long,
